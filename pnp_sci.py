...
def gap_denoise(y, Phi_sum, A, At, _lambda=1, accelerate=True, delta=1.0,
    ...
    k = 0
    count = 0
    for nd, nsig in zip(delta, sigma): # iterate all noise levels
        for it in range(iter_max[count]):
            # if idx == 0:
                # print("initial: ")
                # print(psnr(X_orig, x))
            yb = A(x)
    ...
    if X_orig is not None:
        for imask in range(nmask):
            psnr_.append(peak_signal_noise_ratio(X_orig[...,imask], x[...,imask], data_range=1.))
            ssim_.append(structural_similarity(X_orig[...,imask], x[...,imask], data_range=1.,multichannel=x[...,imask].ndim>2))
...
def admm_denoise(y, Phi_sum, A, At, _lambda=1, gamma=0.01, 
    ...
    psnr_all = []
    k = 0
    for idx, nsig, ngam in zip(itertools.count(), sigma, gamma): # iterate all noise levels
        for it in range(iter_max[idx]):
    ...
    if X_orig is not None:
        for imask in range(nmask):
            psnr_.append(peak_signal_noise_ratio(X_orig[:,:,imask], x[:,:,imask], data_range=1.))
            ssim_.append(structural_similarity(X_orig[:,:,imask], x[:,:,imask], data_range=1.))
    return x, psnr_, ssim_, psnr_all
