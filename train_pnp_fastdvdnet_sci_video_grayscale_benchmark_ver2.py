...
# Parameter setting
max_layers = 60
adam_lr = 0.01  # initial learning parameter for Adam
batch_num = 5
epoch_num = 10
# batch_num = 1
# epoch_num = 140
...
for layer in range(max_layers):
    ...
    layer = 59
    count = 0
    for loop in range(int(epoch_num*len(train)/batch_num)):
        print(layer+1)
        ...
        with open(resultsdir + 'data_files/sigma{}.csv'.format(file_n), 'a') as f:
            ...
            writer = csv.writer(f)
            writer.writerow([loop+1, sigma_list])
        if train_delta == True:
            with open(resultsdir + 'data_files/delta{}.csv'.format(file_n), 'a') as f:
                ...
                writer = csv.writer(f)
                writer.writerow([loop+1, delta_list])
        if train_gamma == True:
            with open(resultsdir + 'data_files/gamma{}.csv'.format(file_n), 'a') as f:
                ...
                writer = csv.writer(f)
                writer.writerow([loop+1, gamma_list])
