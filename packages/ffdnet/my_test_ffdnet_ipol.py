...
from .models import FFDNet
from .my_utils import batch_psnr, normalize, init_logger_ipol, \
				variable_to_cv2_image, remove_dataparallel_wrapper, is_rgb
...
def ffdnet_vdenoiser(vnoisy, sigma, model=None, useGPU=True):
	...
	outv = torch.zeros(vnoisy.shape).to(vnoisy.device)
	for imask in range(nmask):
		...
		# from HxWxC to  CxHxW grayscale image (C=1)
		imnoisy = imnoisy.unsqueeze(0)
		imnoisy = imnoisy.unsqueeze(0)
		# imnoisy = torch.expand_dims(imnoisy, 0)
