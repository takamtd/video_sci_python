...
from packages.ffdnet.my_test_ffdnet_ipol import (ffdnet_vdenoiser, ffdnet_rgb_denoise)
from packages.fastdvdnet.my_test_fastdvdnet import fastdvdnet_denoiser
...
from my_utils import (A_, At_, psnr, denoise_tv)
...
def gap_denoise_bayer(y_bayer, Phi_bayer, _lambda=1, accelerate=True, 
        ...
        for it in range(iter_max[idx]): 
            ...
            if show_iqa and X_orig is not None:
                for ib in range(len(bayer)): 
                    b = bayer[ib]
                    x_bayer[b[0]::2, b[1]::2] = xall[...,ib]
                psnr_all.append(peak_signal_noise_ratio(X_orig, x_bayer,data_range=1.))
                if (k+1)%5 == 0:
    ...
    if X_orig is not None:
        for imask in range(nmask):
            psnr_.append(peak_signal_noise_ratio(X_orig[:,:,imask], x_bayer[:,:,imask], data_range=1.))
            ssim_.append( structural_similarity(X_orig[:,:,imask], x_bayer[:,:,imask], data_range=1.))
...
def admmdenoise_cacti(meas, mask, A, At, projmeth='admm', v0=None, orig=None, 
                      iframe=0, nframe=1, MAXB=1., maskdirection='plain', sigmoid=False,
                      **args):
    '''
    ...
    '''
    nmask = mask.size()[-1]
    mask_sum = torch.sum(mask, axis=tuple(range(2,mask.ndim)))
    
    mask_sum[mask_sum==0] = 1

    x_ = torch.zeros((*mask.shape[:-1],nmask*nframe), dtype=torch.float32)
    ...
    for kf in range(nframe):
        ...
        if projmeth.lower() == 'admm': # alternating direction method of multipliers (ADMM)-based projection
            x_k, psnr_k, ssim_k, psnrall_k = admm_denoise(meas_k, mask_sum, A, At, 
                                                          x0=v0_k, X_orig=orig_k, sigmoid=sigmoid, **args)
    ...
    return x_, psnr_, ssim_, psnrall_

def gap_denoise(y, Phi_sum, A, At, tv_weight=None, _lambda=1, delta=1.0, accelerate=True, 
                denoiser='tv', iter_max=1, noise_estimate=False, sigma=None, 
                tv_iter_max=5, multichannel=True, x0=None, 
                X_orig=None, model=None, show_iqa=True, sigmoid=False, z0=0):
    '''
    ...
    if denoiser.lower() == 'my_tv':
        for it, nweight in zip(iter_max, tv_weight):
            # if idx == 0:
                # print("initial: ")
                # print(psnr(X_orig, x))
            # for nx in range(x.shape[-1]):
            #     x[:,:,nx] = denoise_tv(x[:,:,nx], weight = nweight, max_num_iter=tv_iter_max)
            x = torch.clamp(x, min=-0.5*nweight, max=0.5*nweight)
            yb = A(x)
            if accelerate: # accelerated version of GAP
                y1 = y1 + (y-yb)
        ...
        nmask = x.shape[-1]
        if X_orig is not None:
            X_orig_np = X_orig.to('cpu').detach().numpy().copy()
            x_np = x.to('cpu').detach().numpy().copy()
            for imask in range(nmask):
                psnr_.append(peak_signal_noise_ratio(X_orig_np[...,imask], x_np[...,imask], data_range=1.))
                ssim_.append(structural_similarity(X_orig_np[...,imask], x_np[...,imask], data_range=1.,multichannel=x_np[...,imask].ndim>2))
    ...
    for nd, nsig in zip(delta, sigma):
        yb = A(x)
        if accelerate: # accelerated version of GAP
            if train_delta:
                y1 = y + nd*(y-yb)
...
def admm_denoise(y, Phi_sum, A, At, _lambda=1, gamma=0.01, 
                denoiser='tv', iter_max=60, noise_estimate=False, sigma=None, 
                tv_weight=0.1, tv_iter_max=5, multichannel=True, x0=None, 
                X_orig=None, model=None, show_iqa=True, sigmoid=None, z0=0):
    '''
    Alternating direction method of multipliers (ADMM)[1]-based denoising 
    regularization for snapshot compressive imaging (SCI).
    ...
    for nsig, ngam in zip(sigma, gamma):
        # Euclidean projection
        yb = A(theta+b)
        x = (theta+b) + _lambda*(At((y-yb)/(Phi_sum+ngam))) # ADMM
        # switch denoiser 
        if denoiser.lower() == 'tv': # total variation (TV) denoising
